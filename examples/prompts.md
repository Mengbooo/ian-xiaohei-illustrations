...
```text
Use $ian-xiaohei-illustrations 把下面这篇文章生成 4 张 Bemo 怪诞正文配图。
...
```text
Use $ian-xiaohei-illustrations 这张图方向对，但 Bemo 有点像装饰。
请保持核心意思不变，重生成一版：让 Bemo 成为真正推动结构运转的人。
画面更怪一点，但仍然纯白、清爽、少字。
```
...
```text
Use $ian-xiaohei-illustrations 输出 5 个不同主题的 Bemo 正文配图效果。
